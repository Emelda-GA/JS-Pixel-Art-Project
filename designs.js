...
/*      Code will allow the user to select triangle grid size from sizePicker (grid width input will display available parameters message), select color from colorPicker, and draw art with a click and drag of mouse.
        ...
        const
          pixelWith = 30,
          pixelHeight = Math.round(Math.sqrt(Math.pow(pixelWith, 2) - Math.pow(pixelWith / 2, 2)));
        ...
        function renderPixelGrid() {
          ...
          for (let i = 0; i < gridRows; i++) {
            ...
            for (let j = 0; j < gridCols; j++) {
              ...
              offsetWidth = (pixelWith / 2) * j;
        
              // Switch between Odd and Even Pixels
              if (counter % 2 === 0) {
                pixel.setAttribute('d', `M${( pixelWith / 2 ) + offsetWidth} ${offsetHeight} L${offsetWidth} ${pixelHeight + offsetHeight} L${pixelWith + offsetWidth} ${pixelHeight + offsetHeight} Z`);
              } else {
                pixel.setAttribute('d', `M${offsetWidth} ${offsetHeight} L${( pixelWith / 2 ) + offsetWidth} ${pixelHeight + offsetHeight} L${pixelWith + offsetWidth} ${offsetHeight} Z`);
              }
          ...
          grid.setAttribute('width', (gridCols + 1) * (pixelWith / 2));
          grid.setAttribute('height', gridRows * pixelHeight);
          grid.setAttribute('viewBox', `0 0 ${( gridCols + 1 ) * ( pixelWith / 2 )} ${gridRows * pixelHeight}`);
